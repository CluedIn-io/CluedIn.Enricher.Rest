...
using CluedIn.ExternalSearch.Providers.Rest;
using CluedIn.Providers.Models;
using Constants = CluedIn.ExternalSearch.Providers.Rest.Constants;

namespace CluedIn.Provider.Rest
{
    public class RestSearchProviderProvider : ProviderBase, IExtendedProviderMetadata, IExternalSearchProviderProvider
    {
        public IExternalSearchProvider ExternalSearchProvider { get; }

        public RestSearchProviderProvider([System.Diagnostics.CodeAnalysis.NotNull] ApplicationContext appContext) : base(appContext, GetMetaData())
        {
        ...
        public override async Task<CrawlJobData> GetCrawlJobData(ProviderUpdateContext context, IDictionary<string, object> configuration, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var result = new RestExternalSearchJobData(configuration);
        ...
        {
            if (jobData is RestExternalSearchJobData result)
            {
                return await Task.FromResult(result.ToDictionary());
            }

            throw new InvalidOperationException($"Unexpected data type for {nameof(RestExternalSearchJobData)}, {jobData.GetType()}");
        }
